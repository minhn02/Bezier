...
void test_quintic_curve_evaluation_time_scaled() {
    // initialize vectors
    VectorXd P0(2); P0 << 15, 43;
    VectorXd P1(2); P1 << 1, -5;
    VectorXd P2(2); P2 << 10, -10;
    ...
    assert((curve.evaluate(0) - curve0).norm() < 0.01);
    assert((curve.evaluate(1) - curve01).norm() < 0.01);
    assert((curve.evaluate(2) - curve02).norm() < 0.01);
    assert((curve.evaluate(3) - curve03).norm() < 0.01);
    assert((curve.evaluate(4) - curve04).norm() < 0.01);
    assert((curve.evaluate(5) - curve05).norm() < 0.01);
    assert((curve.evaluate(6) - curve06).norm() < 0.01);
    assert((curve.evaluate(7) - curve07).norm() < 0.01);
    assert((curve.evaluate(8) - curve08).norm() < 0.01);
    assert((curve.evaluate(9) - curve09).norm() < 0.01);
    assert((curve.evaluate(10) - curve1).norm() < 0.01);
}

void test_quintic_curve_evaluation_time_offset() {
    // initialize vectors
    VectorXd P0(2); P0 << 15, 43;
    VectorXd P1(2); P1 << 1, -5;
    VectorXd P2(2); P2 << 10, -10;
    ...
    assert((curve.evaluate(5) - curve0).norm() < 0.01);
    assert((curve.evaluate(5.1) - curve01).norm() < 0.01);
    assert((curve.evaluate(5.2) - curve02).norm() < 0.01);
    assert((curve.evaluate(5.3) - curve03).norm() < 0.01);
    assert((curve.evaluate(5.4) - curve04).norm() < 0.01);
    assert((curve.evaluate(5.5) - curve05).norm() < 0.01);
    assert((curve.evaluate(5.6) - curve06).norm() < 0.01);
    assert((curve.evaluate(5.7) - curve07).norm() < 0.01);
    assert((curve.evaluate(5.8) - curve08).norm() < 0.01);
    assert((curve.evaluate(5.9) - curve09).norm() < 0.01);
    assert((curve.evaluate(6) - curve1).norm() < 0.01);
}

int main(int argc, char const *argv[]) {
    test_cubic_curve_evaluation();
    test_quintic_curve_evaluation();
    test_quintic_curve_evaluation_time_scaled();
    test_quintic_curve_evaluation_time_offset();
}
